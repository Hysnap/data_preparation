...
from scripts.pipeline_code import prepare_combined_df, generate_unique_loc
...
def run_location_enrichment(df, flags, save_to_disk):
    ...
    usstates_df = (
        pd.read_csv("b_source_data/USStates.csv"))
    countrycontinent_df = (
        pd.read_csv("b_source_data/countryContinent.csv"))

    (df_locations,
     df_locations_sum,
     df_unique_locations) = generate_unique_loc(
        df,
        worldcities_df,
        usstates_df,
        countrycontinent_df,
        flags.get("useprelocationenricheddata")
    )
